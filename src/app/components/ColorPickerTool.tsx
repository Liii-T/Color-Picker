  ...
  return (
    <div className="relative shrink-0 size-[40px]">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 40 40">
        <path d={svgSuccessPaths.p328e2800} fill="var(--fill-0, white)" />
      </svg>
    </div>
  );
}
...
const SaturationB = ({ hsv, onChange, currentColor }: { hsv: { h: number; s: number; v: number }; onChange: (newHsv: { h: number; s: number; v: number }) => void; currentColor: string; }) => {
  ...
  return (
    <div className="relative w-full h-[160px] shrink-0">
      <div ref={containerRef} className="absolute inset-0 cursor-pointer touch-none select-none" onPointerDown={handleDown} style={{ backgroundColor: colord({ h: hsv.h, s: 100, v: 100 }).toHex(), backgroundImage: "linear-gradient(to top, #000 0%, rgba(0,0,0,0) 100%), linear-gradient(to right, #FFF 0%, rgba(255,255,255,0) 100%)" }} />
      ...
      <div className="absolute size-[14px] pointer-events-none" style={{ left: `calc(${hsv.s}% - 7px)`, top: `calc(${100 - hsv.v}% - 7px)` }}>
        <div className="absolute inset-[-28.57%]"><svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 22 22"><circle cx="11" cy="11" r="9" stroke="white" strokeWidth="4" /></svg></div>
        <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 14"><circle cx="7" cy="7" fill={currentColor} r="7" /></svg>
      </div>
    </div>
  );
};
...
const HueSliderB = ({ hue, onChange }: { hue: number; onChange: (newHue: number) => void; }) => {
  ...
  return (
    <div className="relative shrink-0 w-full h-[16px]">
      <div ref={containerRef} className="h-[16px] relative rounded-[4px] shrink-0 w-full cursor-pointer touch-none select-none" onPointerDown={handleDown} style={{ backgroundImage: "linear-gradient(90deg, rgb(255, 0, 0) 0%, rgb(255, 255, 0) 16.6%, rgb(0, 255, 0) 33.3%, rgb(0, 255, 255) 50%, rgb(0, 0, 255) 65%, rgb(158, 0, 255) 75%, rgb(255, 0, 167) 89.525%, rgb(255, 0, 0) 100%)" }}>
        <div className="absolute -translate-y-1/2 top-1/2 h-[24px] pointer-events-none flex items-center px-[4px] py-px" style={{ left: `calc(${(hue / 360) * 100}% - 3px)` }}>
          <div className="h-full relative rounded-[4px] shrink-0 w-[6px]"><div aria-hidden="true" className="absolute border-[3px] border-solid border-white inset-0 pointer-events-none rounded-[4px] shadow-[0px_0px_2px_0px_rgba(25,18,24,0.6)]" /></div>
        </div>
      </div>
    </div>
  );
...
export default function ColorPickerTool({ ctx }: { ctx: SkinState }) {
  ...
  // 【修正】只保留一個 initialState 定義
  const [initialState, setInitialState] = useState({
    primary: savedPrimary || DEFAULT_PRIMARY,
    secondary: savedSecondary || DEFAULT_SECONDARY,
  });
  ...
  // 【修正】只保留一個 handleReload 定義，並整合到上方
  const handleReload = () => {
  ...
  const handleSave = () => {
    const defaults = GAME_DEFAULTS[selectedGame];
    const isDefaultColor = primaryColor.toUpperCase() === defaults.primary.toUpperCase() && secondaryColor.toUpperCase() === defaults.secondary.toUpperCase();
    ...
    dialogTimerRef.current = setTimeout(() => { setShowDialog(false); closeSkin(); }, 2000);
  };
  ...
  useEffect(() => { return () => { if (dialogTimerRef.current) clearTimeout(dialogTimerRef.current); }; }, []);
  ...
  const handleBack = () => { if (isDirty) { setShowExitConfirm(true); } else { closeSkin(); } };
  const handleConfirmExit = () => { closeSkin(); };
  const handleCancelExit = () => { setShowExitConfirm(false); };

  return (
    <div className="flex flex-col items-start relative h-full w-full font-sans" style={{ backgroundColor: secondaryColor }}>
      {/* 1. Header */}
      <div className="bg-black flex items-center relative shrink-0 w-full text-white z-20" style={{ paddingTop: 'env(safe-area-inset-top)', height: 'calc(48px + env(safe-area-inset-top))' }}>
        <div className="w-[70px] flex items-center justify-center h-full px-[12px] cursor-pointer active:opacity-60 transition-opacity" onClick={handleBack}><TablerIconX /></div>
        <div className="flex-1 flex justify-center items-center text-[18px] font-medium leading-[normal] text-center">投注皮肤</div>
        <div className="relative shrink-0 w-[70px] h-full" ref={dropdownRef}>
          <button className="flex gap-[7px] h-full items-center justify-end pr-[12px] w-full" onClick={() => setShowDropdown(!showDropdown)}>
            ...
            <div className="flex items-center justify-center"><div className="-scale-y-100 flex-none"><IconDropdownArrow /></div></div>
          </button>
          {showDropdown && (
            <div className="absolute right-0 top-[30px] w-[120px] z-30">
              <div className="flex flex-col items-end justify-end shadow-[0px_1px_12px_0px_rgba(51,51,51,0.08)]">
                <div className="relative shrink-0 size-[18px]"><svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 18 18"><path d={DROPDOWN_CORNER_PATH} fill="white" /></svg></div>
                <div className="flex flex-col gap-px items-start shrink-0">
                  <button className="bg-white flex h-[50px] items-center justify-center w-[120px] shadow-[0px_1px_0px_0px_#bbb39c] hover:bg-gray-50 active:bg-gray-100" onClick={() => handleSwitchGame("快3")}><span className="text-[#333] text-[16px]">快3</span></button>
                  <button className="bg-white flex h-[50px] items-center justify-center w-[120px] shadow-[0px_1px_0px_0px_#bbb39c] hover:bg-gray-50 active:bg-gray-100" onClick={() => handleSwitchGame("其他")}><span className="text-[#333] text-[16px]">其他</span></button>
                </div>
              </div>
            </div>
          )}
        </div>
        {/* 【修正】原本這裡多出一個 </div> 導致白畫面 */}
      </div>
        ...
        {selectedGame === "其他" ? (
          ...
          <>
              ...
              <div className="flex flex-1 items-center justify-center relative w-full" style={{ backgroundColor: primaryColor }}>
                <div className="flex-[1_0_0] h-full relative">
                  <div className="flex flex-col justify-center size-full items-start px-[16px]">
                    <div className="w-full flex justify-center text-[14px] text-center mb-1" style={{ color: previewTextColor }}>0606006期开奖号码</div>
                    <div className="h-[30px] w-full flex justify-center">
                      <div className="flex items-start justify-center pl-[16px] pt-[4px]">
                        {[0, 1, 2].map(i => <div key={i} className="size-[24px] bg-no-repeat" style={{ backgroundImage: `url('${imgItem}')`, backgroundSize: "48px 144px", backgroundPosition: "left top" }} />)}
                        <div className="pl-[8px] flex items-center justify-center"><div className="-scale-y-100 flex-none"><IconCheck color={secondaryColor} /></div></div>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center min-h-[inherit] size-full">
                    <div className="flex items-center min-h-[inherit] pl-[16px] relative size-full">
                      ...
                      <div className="flex flex-col h-full items-center justify-center px-[5px] py-[28px] relative shrink-0 w-[16px]" style={{ backgroundColor: secondaryColor }}>
                        ...
                        <div className="h-[12px] max-w-[16px] overflow-hidden relative shrink-0 w-[6px]"><div className="absolute h-[12px] left-0 overflow-hidden top-0 w-[6px]"><div className="-translate-x-1/2 -translate-y-1/2 absolute h-[12px] left-1/2 top-1/2 w-[6px]"><IconTriangleLeft color={secondaryColor} /></div></div></div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-[1_0_0] flex-col items-start min-h-px py-[12px] relative w-full" style={{ backgroundColor: secondaryColor }}>
              <div className="relative shrink-0 w-full"><div className="flex justify-center w-full"><div className="flex items-start justify-center p-[12px] relative w-full"><div className="flex items-center justify-center relative shrink-0"><div className="-scale-y-100 flex-none"><IconInfo color={secondaryColor} /></div></div><div className="flex flex-[1_0_0] flex-col justify-center ml-2 text-[12px] leading-[normal]" style={{ color: previewTextColor }}>从11-66中任选1个或多个号码，选号與奖号相同即為中獎。賠率11.66倍。</div></div></div></div>
              <div className="relative shrink-0 w-full"><div className="flex gap-[8px] items-start px-[16px] py-[8px] w-full">{[11, 22, 33, 44, 55, 66].map(num => <div key={num} className={cn("flex-[1_0_0] relative border flex justify-center items-center py-[4px]", num <= 33 ? "border-[#f4c829]" : "border-[rgba(255,255,255,0.2)]")}><span className={cn("text-[20px] leading-[32px]", num <= 33 ? "text-[#f4c829]" : "text-white")}>{num}</span></div>)}</div></div>
            </div>
          </>
        )}
      </div>
          ...
          <div className="flex gap-[8px] items-start px-[16px] py-[12px] w-full">
            <div className="bg-[rgba(255,255,255,0.1)] flex-[1_0_0] h-[36px] relative rounded-[4.53px]">
              <div className="flex gap-[4px] items-start p-[4px] size-full relative z-10">
                <button onClick={() => setActiveTab("Primary")} className={cn("flex-[1_0_0] h-full rounded-[4.53px] flex items-center justify-center", activeTab === "Primary" ? "bg-[#575858] text-white" : "text-[rgba(255,255,255,0.5)]")}>主色</button>
                <button onClick={() => setActiveTab("Secondary")} className={cn("flex-[1_0_0] h-full rounded-[4.53px] flex items-center justify-center", activeTab === "Secondary" ? "bg-[#575858] text-white" : "text-[rgba(255,255,255,0.5)]")}>副色</button>
              </div>
            </div>
            <div className="flex gap-[8px] items-center shrink-0">
              <div className="p-[2px] rounded-[99px] size-[32px]" style={{ backgroundColor: currentColor }}><div className="size-full rounded-full border-2 border-white" style={{ backgroundColor: currentColor }} /></div>
              <div className="relative rounded-[4.53px] w-[92px] border border-[#575858] flex items-center px-[12px] py-[8px]"><span className="text-[#969298] text-[14px]">#</span><HexColorInput className="bg-transparent w-full outline-none uppercase font-medium text-[14px] text-white" color={currentColor} onChange={c => currentSetColor(c.startsWith("#") ? c : "#" + c)} /></div>
            </div>
          </div>
        </div>
        <SaturationB hsv={currentHsv} onChange={handleColorChange} currentColor={currentColor} />
        <div className="bg-black p-[16px] w-full"><HueSliderB hue={currentHsv.h} onChange={handleHueChange} /></div>
      </div>

      {/* 4. Footer */}
      <div className="bg-black h-[74px] relative shrink-0 w-full border-t border-[rgba(255,255,255,0.05)]">
        <div className="flex gap-[8px] items-center p-[16px] size-full">
          <button onClick={handleReload} className="bg-[#575858] flex-1 h-full rounded-[4.53px] text-white text-[14px]">恢复默认</button>
          <button onClick={handleSave} className="bg-[#dc3b40] flex-1 h-full rounded-[4.53px] text-white text-[14px]">保存配色</button>
        </div>
      </div>

      {/* 5. Dialogs */}
      {showDialog && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/20" onClick={() => setShowDialog(false)}>
          <div className="bg-[rgba(0,0,0,0.8)] flex flex-col gap-[8px] h-[120px] items-center justify-center rounded-[6px] w-[100px]"><SuccessNoCircle /><p className="text-[12px] text-white">保存成功</p></div>
        </div>
      )}
      ...
      {showExitConfirm && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-[rgba(0,0,0,0.5)]">
          <div className="flex flex-col items-start w-[270px]">
            <div className="bg-white h-[40px] rounded-t-[8px] w-full flex items-center justify-center border-b"><p className="text-[14px] text-black font-medium">温馨提示</p></div>
            <div className="bg-white pb-[20px] pt-[12px] px-[20px] w-full text-center text-[14px] text-black">你已進行投注皮膚配色修改，若直接退出，顏色方案將不會被保存。</div>
            <div className="flex w-full border-t">
              <button className="bg-white flex-1 h-[40px] border-r active:bg-gray-100" onClick={handleCancelExit}><span className="text-[14px] text-black">取消</span></button>
              <button className="bg-white flex-1 h-[40px] active:bg-gray-100" onClick={handleConfirmExit}><span className="text-[14px] text-[#cd3422] font-medium">确定退出</span></button>
            </div>
          </div>
        </div>
      )}
