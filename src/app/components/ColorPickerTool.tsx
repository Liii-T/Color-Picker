...
export default function ColorPickerTool({ ctx }: { ctx: SkinState }) {
  // 1. 從 ctx 讀取記憶狀態 (這部分需要配合 RootLayout 的修改)
  const { 
    setIsCustom, 
    closeSkin, 
    savedPrimary, 
  ...
  // 使用記憶中的顏色作為初始狀態
  const [primaryColor, setPrimaryColor] = useState(savedPrimary || DEFAULT_PRIMARY);
  const [secondaryColor, setSecondaryColor] = useState(savedSecondary || DEFAULT_SECONDARY);
  ...
  const handleSave = () => {
    ...
    // 設定顯示字樣：如果是默認色則 setIsCustom(false)
  ...
  const handleBack = () => {
    ...
    if (isDirty) { setShowExitConfirm(true); } else { closeSkin(); }
  };
