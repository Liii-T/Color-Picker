...
return (
  <div className="flex flex-col h-full w-full font-sans overflow-hidden" style={{ backgroundColor: secondaryColor }}>
    ...
    {/* 1. Header: 固定高度並避開劉海 */}
    <div 
      className="bg-black flex items-center shrink-0 w-full text-white z-20" 
      style={{ 
      ...
      <div className="relative shrink-0 w-[70px] h-full" ref={dropdownRef}>
        ...
        {showDropdown && (
          <div className="absolute right-0 top-[calc(100%-8px)] w-[120px] z-30">
            <div className="flex flex-col items-end shadow-xl bg-white rounded-md overflow-hidden">
              <button className="h-[50px] w-full border-b text-[#333]" onClick={() => { setLayout("A"); setShowDropdown(false); }}>A</button>
              <button className="h-[50px] w-full text-[#333]" onClick={() => { setLayout("B"); setShowDropdown(false); }}>B</button>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="flex-1 flex flex-col w-full relative overflow-hidden" style={{ backgroundColor: secondaryColor }}>
      ...
      <div className="flex flex-col h-[68px] shrink-0 w-full relative">
        ...
        <div className="flex flex-1 items-center justify-center" style={{ backgroundColor: primaryColor }}>
          {/* ... 開獎號碼內容 ... */}
          <div className="flex flex-col items-center">
             <div className="text-[14px]" style={{ color: previewTextColor }}>0606006期开奖号码</div>
             <div className="flex gap-1">
                <div className="size-[24px]" style={{ backgroundImage: `url('${imgItem}')`, backgroundSize: 'contain' }} />
                <div className="size-[24px]" style={{ backgroundImage: `url('${imgItem}')`, backgroundSize: 'contain' }} />
                <div className="size-[24px]" style={{ backgroundImage: `url('${imgItem}')`, backgroundSize: 'contain' }} />
             </div>
          </div>
      ...
      {/* Main Body Preview */}
      <div className="flex-1 flex flex-col items-start py-[12px] overflow-hidden">
        {/* 提示文字 */}
        <div className="p-[12px] flex items-start gap-2" style={{ color: previewTextColor }}>
          <IconInfo color={secondaryColor} />
          <div className="text-[12px]">从11-66中任选1个或多个号码...</div>
        </div>
        
        {/* 號碼格 */}
        <div className="grid grid-cols-6 gap-2 px-[16px] w-full">
          {[11, 22, 33, 44, 55, 66].map((num) => (
            <div key={num} className="border flex justify-center py-2 rounded" style={{ borderColor: num <= 33 ? '#f4c829' : 'rgba(255,255,255,0.2)' }}>
              <span style={{ color: num <= 33 ? '#f4c829' : 'white' }}>{num}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    {/* 3. Footer 控制面板: 根據 Layout A/B 顯示 */}
    <div className="shrink-0 w-full bg-[#252625] rounded-t-[12px] z-20">
      {layout === "A" ? (
        <div className="flex flex-col">
          <div className="p-[16px] flex flex-col gap-4">
            {/* 分段切換 */}
            <div className="bg-white/5 p-1 rounded-lg flex gap-1">
              <button onClick={() => setActiveTab("Primary")} className={cn("flex-1 py-1.5 rounded", activeTab === "Primary" ? "bg-[#575858] text-white" : "text-gray-400")}>主色</button>
              <button onClick={() => setActiveTab("Secondary")} className={cn("flex-1 py-1.5 rounded", activeTab === "Secondary" ? "bg-[#575858] text-white" : "text-gray-400")}>副色</button>
            </div>
          ...
          {/* 按鈕列: 避開底部黑條 */}
          <div 
            className="border-t border-black/30 p-[16px] flex gap-2" 
        ...
        <div className="flex flex-col">
          {/* Layout B 的內容，同樣要記得加 env(safe-area-inset-bottom) */}
