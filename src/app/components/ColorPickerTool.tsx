...
export default function ColorPickerTool({ ctx }: { ctx: SkinState }) {
  ...
  // 1. 初始化讀取 ctx 裡的保存色作為髒檢查起點 (刪除重複定義，只保留這一個)
  ...
  const handleSave = () => {
    ...
    setInitialState({ primary: primaryColor, secondary: secondaryColor }); // 存檔後重置 initialState
  ...
  const handleSwitchGame = (game: string) => {
    ...
    if (defaults) {
      setPrimaryColor(defaults.primary);
      setSecondaryColor(defaults.secondary);
      // 切換遊戲時視為載入新的預設環境，通常建議重置 initialState
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col w-full relative overflow-hidden justify-between" style={{ backgroundColor: secondaryColor }}>
        {selectedGame === "其他" ? (
          ...
          <>
                  ...
                  <div className="flex items-center min-h-[inherit] size-full">
                    <div className="flex items-center min-h-[inherit] pl-[16px] relative size-full">
                      <div className="flex flex-[1_0_0] flex-col h-full items-center justify-center pt-[2px]">
                        <div className="text-[14px] text-center" style={{ color: previewTextColor }}>
                          0606007期投注截止
                        </div>
                        <div className="text-[24px] text-center leading-[30px]" style={{ color: previewTextColor }}>
                          00:08:23
                        </div>
                      </div>
                      <div
            ...
            <div className="flex flex-[1_0_0] flex-col items-start min-h-px py-[12px] relative w-full">
              <div className="relative shrink-0 w-full p-[12px] flex">
                <IconInfo color={secondaryColor} />
                <div className="ml-2 text-[12px]" style={{ color: previewTextColor }}>
                  从11-66中任选1个或多个号码，选号與奖号相同即中奖。
                </div>
              </div>
              <div className="flex gap-[8px] px-[16px] py-[8px] w-full">
                {[11, 22, 33].map((num) => (
                  <div key={num} className="flex-1 border border-[#f4c829] flex justify-center py-[4px]">
                    <span className="text-[#f4c829] text-[20px]">{num}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="flex flex-col items-center relative shrink-0 w-full">
        <div className="bg-black px-[16px] py-[12px] w-full flex gap-[8px]">
          <div className="bg-[rgba(255,255,255,0.1)] flex-1 h-[36px] rounded-[4.53px] flex p-[4px] gap-[4px]">
            <button onClick={() => setActiveTab("Primary")} className={cn("flex-1 h-full rounded-[4.53px] flex items-center justify-center transition-all", activeTab === "Primary" ? "bg-[#575858] text-white" : "text-[rgba(255,255,255,0.5)]")}>
              <span className="text-[14px]">主色</span>
            </button>
            <button onClick={() => setActiveTab("Secondary")} className={cn("flex-1 h-full rounded-[4.53px] flex items-center justify-center transition-all", activeTab === "Secondary" ? "bg-[#575858] text-white" : "text-[rgba(255,255,255,0.5)]")}>
              <span className="text-[14px]">副色</span>
            </button>
          </div>

          <div className="flex gap-[8px] items-center">
            <div className="size-[32px] rounded-full border-2 border-white" style={{ backgroundColor: currentColor }} />
            <div className="relative rounded-[4.53px] w-[92px] border border-[#575858] px-[12px] py-[8px] flex text-white text-[14px]">
              <span className="text-[#969298] mr-1">#</span>
              <HexColorInput className="bg-transparent w-full outline-none uppercase" color={currentColor} onChange={(c) => currentSetColor(c.startsWith("#") ? c : "#" + c)} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-black p-[16px] w-full">
          <HueSliderB hue={currentHsv.h} onChange={handleHueChange} />
        </div>
      </div>

      {/* 4. Footer */}
      <div className="bg-black h-[74px] w-full flex items-center p-[16px] gap-[8px] border-t border-white/5">
        <button onClick={handleReload} className="bg-[#575858] flex-1 h-full rounded-[4.53px] text-white">恢復默認</button>
        <button onClick={handleSave} className="bg-[#dc3b40] flex-1 h-full rounded-[4.53px] text-white">保存配色</button>
      </div>

      {/* 5. Dialogs */}
      {showDialog && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-[rgba(0,0,0,0.8)] flex flex-col gap-[8px] h-[120px] items-center justify-center rounded-[6px] w-[100px]">
            <SuccessNoCircle />
            <p className="text-[12px] text-white">保存成功</p>
          </div>
        </div>
      )}
      ...
      {showExitConfirm && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-[rgba(0,0,0,0.5)]">
          <div className="bg-white rounded-xl w-[270px] overflow-hidden">
            <div className="h-[40px] flex items-center justify-center font-medium border-b text-black text-[14px]">温馨提示</div>
            <div className="p-[20px] text-center text-black text-[14px]">你已进行配色修改，若直接退出，顏色方案將不會被保存。</div>
            <div className="flex border-t">
              <button className="flex-1 h-[40px] border-r active:bg-gray-100 text-black text-[14px]" onClick={handleCancelExit}>取消</button>
              <button className="flex-1 h-[40px] active:bg-gray-100 text-[#cd3422] font-medium text-[14px]" onClick={handleConfirmExit}>确定退出</button>
            </div>
          </div>
        </div>
      )}
